...
class NeuralNetwork:
    ...
    Args:
        ...
        learning_rate (float): the learning rate, should be between -1 and 1
    """
    ...
    def __init__(self, inputsize, layersizes, outputsize, learning_rate=0.5):
        ...
        for i in range(1,len(layersizes)):
            self.weights.append( 2*random.random( size=(layersizes[i-1], layersizes[i]) ).T-1 )
...
def main(input_size, output_size, layers=[5], traincycles=5000, 
    ...
    for i in range(traincycles):
        ...
        if i%10 == 0:
            history.append(copy.deepcopy(nn))
    ...
    # animate evolution of the network during learning
    animate(history)
...
if __name__ == "__main__":
    ...
    main(1,1, layers=[9,9], 
        traincycles=training_time,
        trainfile="sin-trainingdata.csv", testfile="sin-testdata.csv")
