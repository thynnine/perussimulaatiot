...
def berendsen_thermostat(particles, dt, tau = 5.0, t0 = 0.1):
    ...
    t = calculate_temperature(particles)
    scaler = sqrt( 1 + dt/tau*( t0/t - 1 ) )
    scale_velocities(particles, scaler)
...
def run_series(filename, min_t, max_t, tau, dt, sample_interval, simulation_time, thermalization_time):
    ts = []
    ps = []
    tes = []
    pes = []
    ...
    for T in np.linspace(min_t, max_t, int((max_t -min_t)/0.01 + 1.1) ):
        ...
        expand_supercell(particles, box, 2)
        # Initialize the velocities of particles.
        assign_maxwell_boltzmann_velocities(particles, min_t)
        ...
        for i in range(n_samples):
            ...
            t, p = velocity_verlet(particles, box, dt, sample_interval, 1.0, T, tau)
            print_progress(i+1, n_samples)
    ...
    plt.clf()
    plt.errorbar(ts, ps, xerr=tes, yerr=pes, marker='o', ms=3)
    plt.show()


if __name__ == "__main__":

    filename = 'fcc_compress.txt'
    T = 0.1
    tau = 5
    dt = 0.02
    sample_time = 1.0
    ...
    #run_series(filename, 0.01, 0.20, tau, dt, sample_time, simulation_time, thermalization_time)
